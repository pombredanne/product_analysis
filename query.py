...
def scheduled_query(pkey, prkey, ai, pid,  start, end, chart_ids, interval ):
    ...
    for cid in chart_ids:
        try:
            result = query_request(cid, start, end, prkey, code, interval)
...
if __name__ == "__main__":
    ...
    endDate = str(int(datetime.datetime(2017, 7, 2).timestamp() * 1000))
    startDate = str(int(datetime.datetime(2017, 7, 1).timestamp() * 1000 -1))
    ...
    t = Timer(mins * 60, scheduled_query, [pkey, prkey, ai, pid, startDate, endDate, chart_ids, interval],)
    t.start()
