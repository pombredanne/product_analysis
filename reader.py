...
user_org_info_df = pd.read_csv("./user_org_info.txt",dtype="str")
# headers = ["时间","user","主要功能——单图细节（分析师专用勿改）_页面浏览量","fqy-单图列表页面_页面浏览量","fqy-创建单图页面_页面浏览量",	"单图-编辑页面(分析师专用勿改)_页面浏览量	看板全部页面（分析师专用勿改）_页面浏览量	fqy-创建看板页面_页面浏览量	看板-编辑页面(分析师专用勿动)_页面浏览量","fqy-看板列表页面(勿动)_页面浏览量	主要功能——漏斗全部_页面浏览量","主要功能——留存全部页面_页面浏览量","主要功能——用户细查_页面浏览量","主要功能——实时_页面浏览量","热图使用情况用这个指标_浏览量","新建——漏斗_页面浏览量"]
parse_dates = ['时间']

user_use_data_df = pd.read_csv("./1228-0327 FQY_主要功能数据_U_user_table_PV浏览类.txt",sep="\t",dtype="str",parse_dates=parse_dates)
user_session_duration_df = pd.read_csv("./1201-0330 user_访问量&访问时长.csv",dtype="str",parse_dates=parse_dates)
user_session_duration_df["year"] = user_session_duration_df["时间"].dt.year
user_session_duration_df["week_num"] = user_session_duration_df["时间"].dt.week


user_visit_and_session_duration_df 	= pd.read_csv("./1228-0327 FQY_主要功能数据_U_user_table_action交互类.txt",sep="\t",dtype="str", parse_dates=parse_dates)

tmp1 = pd.merge(user_data_sim_df,user_session_duration_df, how="left", left_on=["user_id","year","week_num"],right_on=["user","year","week_num"])
tmp2 = pd.merge(tmp1,user_org_info_df,how="inner",left_on="user_id",right_on="user_id")

dfs = [tmp2,user_use_data_df,user_visit_and_session_duration_df]

result = reduce(lambda left, right: pd.merge(left,right,how="left",left_on=["user_id","时间"],right_on=["user","时间"]), dfs)
